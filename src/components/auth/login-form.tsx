...
export function LoginForm({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const { isValid, value: sanitizedIdentifier } = validateIdentifier(identifier);
    if (!isValid) {
      return setFormError('O e-mail ou documento informado é inválido');
    }

    setLocalLoading(true);
    try {
      await login({ identifier: sanitizedIdentifier, password });
        ...
        <Field>
          <div className="flex items-center">
            ...
            <Link
              ...
              className="ml-auto text-xs font-medium text-primary underline-offset-4 hover:underline transition-colors"
            >
              Esqueceu a senha?
            </Link>
          </div>
