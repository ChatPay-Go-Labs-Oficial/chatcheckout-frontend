...
export function StellarPaymentReview({ product, checkout }: StellarPaymentReviewProps) {
    ...
    // Verificar se produto já está em USDC
    const isProductInUSDC = product.currency?.toUpperCase() === 'USDC';

    // Só fazer conversão se produto estiver em BRL
    const { convertedAmount, exchangeRate, isLoading } = usePriceConversion(
        isProductInUSDC ? 0 : product.price, // Se já é USDC, não converte
        'BRL',
        'USDC'
    );

    // Valor final em USDC
    const amountUSDC = isProductInUSDC ? product.price : convertedAmount;

    // Cálculo de taxas
    const fees = useMemo(() => {
        const networkFee = STELLAR_FEES.BASE_FEE_XLM; // ~0.00001 XLM (desprezível em USDC)
        const spreadAmount = amountUSDC * STELLAR_FEES.SPREAD_PERCENT;
        const total = amountUSDC + networkFee + spreadAmount;
            ...
            <div className="bg-white rounded-xl shadow-sm overflow-hidden border border-gray-100">
                ...
                <div className="p-4 space-y-4">
                    ...
                    <div className="space-y-2 text-sm">
                        {/* Se produto já é USDC, mostrar direto */}
                        {isProductInUSDC ? (
                            <>
                                ...
                                <div className="flex justify-between items-center text-xs">
                                    ...
                                    <span className="text-gray-600">~{fees.spreadAmount.toFixed(2)} USDC</span>
                                </div>
                                ...
                                <div className="flex justify-between items-center text-base font-semibold">
                                    ...
                                    <span className="text-blue-600">{fees.total.toFixed(2)} USDC</span>
                                </div>
                            </>
                        ) : (
                            <>
                                ...
                                {isLoading ? (
                                    ...
                                    <>
                                        <div className="flex justify-between items-center text-xs text-gray-500">
                                            ...
                                            <span>1 USDC = R$ {exchangeRate.toFixed(2)}</span>
                                        </div>

                                        <div className="border-t border-gray-100 pt-2 mt-2"></div>

                                        <div className="flex justify-between items-center">
                                            <span className="text-gray-600">Valor em USDC:</span>
                ...
                <div className="p-4">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <p className="text-sm font-semibold text-gray-900">USDC (Stellar)</p>
                                <p className="text-xs text-gray-500">Pagamento via criptomoeda</p>
                            </div>
                        </div>
