...
const ProductImage = ({
  ...
  if (!src || error) {
    return (
      <div
        className={cn(
          'aspect-video w-full bg-muted/10 flex flex-col items-center justify-center shrink-0',
          className,
        )}
      >
      ...
      {view === 'grid' ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-4 auto-rows-fr">
          {products.map((product) => (
            <Card
              key={product.id}
              className="group flex flex-col border-muted/60 transition-colors shadow-sm overflow-hidden gap-0 py-0 h-full"
            >
              ...
              <CardFooter className="flex items-center justify-between px-4 py-2.5 bg-muted/50 border-t shrink-0">
                <div className="flex items-center gap-1.5 min-w-0">
                  ...
                  {product.currency !== 'BRL' && (
                    <Badge
                      variant="outline"
                      className="text-[9px] h-4 px-1 bg-background/50 border-muted-foreground/20 font-medium whitespace-nowrap"
                    >
                      {product.currency}
                    </Badge>
                  )}
                  ...
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                      <Button
                        variant="ghost"
                        size="icon"
                        className="h-8 w-8 p-0 rounded-md hover:bg-background/80"
                      >
          ...
          <Table className="min-w-[650px]">
            <TableHeader>
              <TableRow className="hover:bg-transparent bg-muted/50">
                <TableHead className="px-4 h-11 text-[10px] font-bold text-muted-foreground uppercase tracking-wider w-[160px]">
                  Produto
                </TableHead>
                <TableHead className="px-4 h-11 text-[10px] font-bold text-muted-foreground uppercase tracking-wider text-left whitespace-nowrap w-[140px]">
                  Preço
                </TableHead>
                <TableHead className="px-4 h-11 text-[10px] font-bold text-muted-foreground uppercase tracking-wider text-left whitespace-nowrap w-[140px]">
                  Moeda
            ...
            <TableBody>
              {products.map((product) => (
                <TableRow
                  key={product.id}
                  className="group transition-colors border-b last:border-0 hover:bg-muted/10"
                >
                  <TableCell className="px-4 py-3.5">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex flex-col min-w-0">
                        <span
                          className="font-semibold text-sm text-foreground truncate leading-none"
                          title={product.name}
                        >
                          {product.name}
                        </span>
                  ...
                  <TableCell className="px-4 py-3.5 text-left">
                    <Badge
                      variant="outline"
                      className="text-[10.5px] px-2 py-0 h-5 bg-muted/60 text-foreground font-bold border-muted-foreground/30 shadow-none"
                    >
                      {product.currency}
                    </Badge>
                  </TableCell>
        ...
        <DialogContent className="sm:max-w-md rounded-lg">
          <DialogHeader>
            ...
            <DialogDescription className="text-sm">
              ...
              <span className="font-semibold text-foreground">{productToDelete?.name}</span>? Esta
