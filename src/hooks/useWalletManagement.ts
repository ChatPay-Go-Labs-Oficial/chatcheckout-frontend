...
export function useWalletManagement() {
    ...
    const handleReleaseBatch = useCallback(async () => {
        ...
        try {
            ...
            const canUsePasskeyPath = passkeyWallet.isConnected && sourceAddress === passkeyFeePayerAddress;
            toast.info(releasableEscrows.length > 1 ? `Processando ${releasableEscrows.length} resgates...` : 'Processando resgate...');
            ...
            if (result.failed === 0) {
                toast.success(result.success === 1 ? 'Recebimento realizado!' : `${result.success} processados.`);
            } else {
    ...
    }, [passkeyWallet, passkeyFeePayerAddress, releasableEscrows, refreshReleasableEscrows, sellerAddress, signerWallet, toast]);
